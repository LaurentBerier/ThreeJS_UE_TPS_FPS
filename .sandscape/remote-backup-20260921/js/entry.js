...
import JourneyProgress from './entities/UI/JourneyProgress.js'
...
class FPSGameApp{
  ...
  async LoadAssets(){
    ...
    // Sandscape capture: nobody presses START in a headless browser, so the
    // export param starts the game itself. It has to be CALLED here — putting
    // the check inside StartGame only skips the fades of a function nothing
    // invokes, and the capture then waits forever on a title screen.
    // Dev-only: the shipping path still waits for the click.
    if(new URLSearchParams(location.search).has('sandscape-export')){
      this.StartGame();
    }
  }
  ...
  // Sandscape Level Editor round trip. Loaded BEFORE the world builds; null (or a failed
  // fetch) leaves the game building exactly its own layout. ?level=off forces the built-in
  // layout; ?level=<path> points at another scene.
  async LoadEditedLevel(){
    const params = new URLSearchParams(location.search)
    if(params.get('level') === 'off'){ return null }
    try{
      const mod = await import('./sandscape/level-import.js')
      const imported = await mod.loadEditedLevel({ candidates: ['scenes/journey_level.json'] })
      // Kept for EntitySetup's parity check and for WorldProps' transform application. Held here
      // rather than imported at the top of the file because js/sandscape/ is provisioned only once
      // a game round-trips a level — a static import would turn a missing module into a game that
      // does not boot, and this path must fail soft. Threading the functions down is what lets
      // WorldProps use the shared helpers without taking that risk.
      this.auditImportedLevel = mod.auditImportedLevel
      this.applyPlacement = mod.applyPlacement
      if(imported){ console.log(`[level-import] scenes/journey_level.json: ${imported.placements ? imported.placements.length : 0} placements`) }
      return imported
    }catch(err){
      console.warn('[level-import] unavailable, using the built-in layout:', err)
      return null
    }
  }
  ...
  async EntitySetup(){
    ...
    // The edited scene (if any) must be in hand before the world builds.
    this.importedLevel = await this.LoadEditedLevel();
    ...
    const worldProps = new WorldProps(this.scene, terrain, propScenes, this.importedLevel, this.applyPlacement);
    // Round trip: WorldProps' constructor branches on the edited scene — the import REPLACES the
    // authored placement table (never alongside it). Fails soft: nothing placed falls back.
    if(this.importedLevel){
      this.importedPlaced = worldProps.importedPlaced;
      // Parity audit, from the runtime module rather than a count written here. A COUNT cannot see
      // the way this adapter actually broke: assigning `rotation.order` after `quaternion.set()`
      // left 20 of the 76 props at a different rotation than the document asks for (worst 165
      // degrees) while every count read healthy. auditImportedLevel compares what the game BUILT
      // against what the document ASKED FOR and reports each class of mismatch with a count and a
      // worst-case magnitude (DOUBLE-SPAWN / SHORT PLACEMENT / WRONG ROTATION / WRONG POSITION),
      // which is what makes the failure readable from the console instead of from a screenshot.
      //
      // Gated on importedPlaced: when the import places nothing, WorldProps falls back to the
      // AUTHORED layout — which reproduces the document's own transforms almost exactly, because
      // the document was captured from it. Auditing that reports `parity ok` on a level that
      // ignored the import entirely, one line after the fallback warning says the opposite.
      //
      // The whole scene, not worldProps.root: level-export.js walks the whole scene when it
      // captures, so anything tagged outside WorldProps must be visible to the audit too, or
      // DOUBLE-SPAWN goes unseen on the day someone tags a prop somewhere else.
      // matrixWorld must be current first — every imported node has matrixAutoUpdate false.
      if(this.importedPlaced && this.auditImportedLevel){
        this.scene.updateMatrixWorld(true);
        this.auditImportedLevel(this.scene, this.importedLevel);
      }else if(this.importedPlaced){
        console.error('[level-import] audit unavailable — js/sandscape/level-import.js is stale ' +
          '(no auditImportedLevel export). The level was placed but NOT verified; re-provision ' +
          'the runtime modules before trusting this import.');
      }
    }
    levelEntity.AddComponent(worldProps);
    ...
    beastLocations.forEach((v,i)=>{
      ...
      // THE BOSS (the LAST beast location = the summit arena): the finale duel. The wreck-field
      // beast keeps the class defaults (20 HP / 35 dmg — a spike, not a war); the boss gets the
      // full 100 HP and a 45 HP punch, so the citadel fight is a real one (see also
      // CharacterController's BOSS EXCEPTION comment). Set BEFORE Initialize (EndSetup) so the
      // controller's own defaults are overridden before its FSM first reads them.
      if(i === beastLocations.length - 1){
        const bossCtrl = npcEntity.GetComponent('CharacterController');
        bossCtrl.health = 100;
        bossCtrl.maxHealth = 100;
        bossCtrl.meleeDamage = 45;
      }
    ...
    // Journey progress + goals: the readable campaign layer over the authored level — route
    // progress along the spine, encounter CONTACT/CLEAR announcements, the citadel finale when
    // the boss's encounter clears, and the death overlay. Read-only over AI/physics state.
    const progressEntity = new Entity();
    progressEntity.SetName("JourneyProgress");
    progressEntity.AddComponent(new JourneyProgress());
    this.entityManager.Add(progressEntity);
    ...
    // Parity line: one log summarising level construction, so an imported scene vs the built-in
    // layout is a console diff, not a playtest.
    const footN = (structures.footprints || []).length;
    console.log(`[Journey] layout parity: props=${worldProps.root.children.length} colliders=${footN} ` +
      `soldiers=${SPAWNS.soldiers.length} beasts=${SPAWNS.beasts.length + 1} ammo=${SPAWNS.ammo.length} ` +
      `imported=${this.importedPlaced ? 'yes' : 'no'}`);

    // Sandscape level export: only armed under ?sandscape-export (never on the shipping path).
    // The journey level is gated behind the START button, so the hook auto-starts the game.
    if(new URLSearchParams(location.search).has('sandscape-export')){
      try{
        const { installLevelExport } = await import('./sandscape/level-export.js');
        const { CASTLE } = await import('./entities/World/JourneyWorld.js');
        const A = CASTLE.arena;
        await installLevelExport({
          scene: this.scene, camera: this.camera, renderer: this.renderer,
          name: 'Journey Level',
          heightAt: (x, z) => terrain.HeightAt(x, z),
          // The ground's colour comes from the DesertLook shader grade (GROUND_BASE_TINT deep-red
          // hue × GROUND_DARK luma), not material.color — sample the RENDERED look, not the material.
          terrain: { size: 640, segments: 64, color: 0x3a1410, roughness: 1 },
          // Carry the Venice-sunset HDR into the editor so the level doesn't open against a void.
          environment: { path: 'assets/hdri/venice_sunset_1k.hdr', background: true },
          includeLight: (l) => l.userData.sandscapeLevelLight === true,
          markers: [
            { name: 'Player spawn', position: new THREE.Vector3(SPAWNS.player.x, terrain.HeightAt(SPAWNS.player.x, SPAWNS.player.z), SPAWNS.player.z),
              data: { kind: 'player-spawn', yaw: SPAWNS.player.yaw } },
            { name: 'Boss spawn', position: new THREE.Vector3(SPAWNS.boss.x, terrain.HeightAt(SPAWNS.boss.x, SPAWNS.boss.z), SPAWNS.boss.z),
              data: { kind: 'enemy-spawn', enemy: 'boss' } },
            ...SPAWNS.soldiers.map((s, i) => ({ name: `Soldier ${i}`, position: new THREE.Vector3(s.x, terrain.HeightAt(s.x, s.z), s.z),
              data: { kind: 'enemy-spawn', enemy: 'soldier' } })),
            ...SPAWNS.beasts.map((s, i) => ({ name: `Beast ${i}`, position: new THREE.Vector3(s.x, terrain.HeightAt(s.x, s.z), s.z),
              data: { kind: 'enemy-spawn', enemy: 'beast' } })),
            ...SPAWNS.ammo.map((s, i) => ({ name: `Ammo ${i}`, position: new THREE.Vector3(s.x, terrain.HeightAt(s.x, s.z), s.z),
              data: { kind: 'ammo' } })),
            { name: 'Exit (summit arena)', position: new THREE.Vector3(A.x, A.floor, A.z), data: { kind: 'exit' } },
          ],
        });
      }catch(err){ console.error('[sandscape-export] failed:', err); }
    }
  ...
  StartGame = async ()=>{
    ...
    this.starting = true;
    // Sandscape capture: nobody presses START in a headless browser — auto-start under the
    // export param. Dev-only; the shipping path still waits for the click.
    if(new URLSearchParams(location.search).has('sandscape-export')){
      this.ShowMenu(false);
