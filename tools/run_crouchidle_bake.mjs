// One-shot headless bake of the crouch-idle clip into
// assets/characters/ue/CrouchIdle.glb. Self-contained: serves the project root over
// http, drives tools/crouchidle_to_glb.html in the cached Chrome for Testing via
// puppeteer-core, and writes the GLB the page produces.
//
//   node tools/run_crouchidle_bake.mjs
//
...
const OUT = join(ROOT, 'assets', 'characters', 'ue', 'CrouchIdle.glb');
...
const server = http.createServer((req, res) => {
  ...
  if (p === '/') p = '/tools/crouchidle_to_glb.html';
...
const browser = await puppeteer.launch({
  ...
  args: [
    ...
    '--user-data-dir=/tmp/crouchidle-bake-chrome',
  ],
});
try {
  ...
  const url = `http://127.0.0.1:${PORT}/tools/crouchidle_to_glb.html`;
