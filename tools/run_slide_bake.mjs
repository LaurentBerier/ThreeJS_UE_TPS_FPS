// One-shot headless bake of the forward-slide clip into
// assets/characters/ue/Slide.glb. Self-contained: serves the project root over http,
// drives tools/slide_to_glb.html in the cached Chrome for Testing via puppeteer-core,
// and writes the GLB the page produces.
//
//   node tools/run_slide_bake.mjs
//
// Needs network (the page pulls three@0.160 from unpkg) and a Chrome binary: uses
// $CHROME_BIN if set, else the puppeteer cache's Chrome for Testing. On a machine without
// that Chrome (e.g. Windows here), use the pure-Node twin instead: d:/tmp/jogcheck/bake_slide.mjs
// (loads SK_Mannequin.glb bones, pulls + flattens the slide clip, exports Slide.glb — no browser).
...
const OUT = join(ROOT, 'assets', 'characters', 'ue', 'Slide.glb');
...
const server = http.createServer((req, res) => {
  ...
  if (p === '/') p = '/tools/slide_to_glb.html';
...
let exe = findChrome();
if (!exe) {
  console.error('Could not locate Chrome for Testing; set CHROME_BIN (or use d:/tmp/jogcheck/bake_slide.mjs).');
  process.exit(2);
}
...
const browser = await puppeteer.launch({
  ...
  args: [
    ...
    '--user-data-dir=/tmp/slide-bake-chrome',
  ],
});
try {
  ...
  const url = `http://127.0.0.1:${PORT}/tools/slide_to_glb.html`;
