// Headless build step: drive tools/ue_fbx_to_glb.html in Edge to produce
// assets/characters/ue/SK_Mannequin.glb (mesh + 4 clips). Requires the dev
// server running (python serve.py) and puppeteer-core available.
//
//   node tools/convert_ue.mjs            (expects server on :8070)
//   PORT=8071 node tools/convert_ue.mjs
//
...
const { default: puppeteer } = await import(join(puppeteerDir, 'lib', 'esm', 'puppeteer-core.js'))
  .catch(() => import('puppeteer-core'));

const url = `http://127.0.0.1:${PORT}/tools/ue_fbx_to_glb.html`;
const outPath = join(__dir, '..', 'assets', 'characters', 'ue', 'SK_Mannequin.glb');
